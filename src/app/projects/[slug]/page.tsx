...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="min-h-screen pt-32 pb-20 px-4 md:px-12 max-w-[1000px] mx-auto">
      {/* Navigation */}
      <div className="mb-12">
        <Link href="/projects" className="inline-flex items-center gap-2 text-text-secondary hover:text-accent transition-colors group">
      ...
      <div className="grid md:grid-cols-[1fr_2fr] gap-12 md:gap-24 mb-24">
        ...
        <div className="prose prose-invert prose-lg text-text-secondary">
          <p className="whitespace-pre-wrap leading-relaxed">{project.overview || project.description}</p>
        </div>
      </div>
      ...
      <div className="border-t border-border pt-24">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {relatedProjects.map((p) => (
            <Link href={`/projects/${p.slug}`} key={p.id} className="group bg-bg-card border border-border rounded-lg overflow-hidden hover:border-accent transition-all duration-300">
        ...
        <div className="mt-12 text-center">
             <Link href="/projects" className="inline-flex items-center gap-2 text-text-secondary hover:text-accent transition-colors font-medium">
